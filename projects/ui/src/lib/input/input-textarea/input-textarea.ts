...
const inputTextareaVariants = cva(
  [
    'flex min-h-20 w-full rounded-md border px-3 py-2 text-sm transition-colors',
...
export class InputTextarea implements FormValueControl<string> {
  ...
  public readonly rows = input<number>(3, { transform: numberAttribute });
  public readonly uiClass = input('');

  protected readonly inputId = computed(
    () => this.id() ?? `ui-input-textarea-${InputTextarea.nextId++}`,
  );
  protected readonly hasErrors = computed(() => this.errors().length > 0);

  protected readonly inputClasses = computed(() =>
    cn(
      inputTextareaVariants({ fullWidth: this.fullWidth() }),
      this.hasErrors() &&
